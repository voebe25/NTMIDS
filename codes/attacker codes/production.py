...
class scanclass(threading.Thread): #Thread that will handle all arp scan till below class prints spinner.
    ...
    def run(self):
        ...
        '''row_count = 0
        col_count = 0
        print "length of agents sys --",len(agent_systems)
        for i in range(1,len(agent_systems)+1):
           if (i % 10) is 0:
              row_count += 1
              col_count = 0
           btn = Button(reference,text="user"+str(i),command=(lambda i=i:self.user_details(i-1)))
    ...
    def run(self):
        ...
        for i in values:
            ...
            if not present:
                    ...
                    ip_scan_text.tag_config("( No Agent )",foreground="red")
                    pos = str(counter)+".0"
                    ip_scan_text.image_create(END,image=self.photoImg)
                    ip_scan_text.insert(END,str("   "+i))
...
def main():
   global report_http,report_https,report_icmp,report_arp,report_dnsqry,report_udp,report_dhcp,report_ftp,report_telnet,report_ssh,traffic_text,ip_scan_text,agent_systems
   report_http = 0
   report_https = 0
   report_icmp = 0
   report_arp = 0
